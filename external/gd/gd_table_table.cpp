...
*/
table::table( detail::columns* pcolumns, const table_column_buffer* ptable, uint64_t uFrom, uint64_t uCount )
   : m_puData{}
{
 ...
 * @param uCount number of rows to copy
*/
table::table( detail::columns* pcolumns, const table* ptable, uint64_t uFrom, uint64_t uCount )
   : m_puData{}
...
{                                                                                                  assert( m_pcolumns->empty() == false ); assert( m_puData == nullptr );
   ...
   {                                                                                               assert( uRowSize % SPACE_ALIGN == 0 );// each value is aligned (4 byte alignment ?) for better performance
      ...
      {
         uState |= gd::table::table::eColumnStateLength;
      ...
      else
      {
         ...
         else
         {
            ...
            uState |= gd::table::table::eColumnStateReference;   // for reference values length is store in reference object and index to reference is stored as cell value in table
         }
      }
 ...
 * @param uCount number of rows to add
*/
void table::append( const table_column_buffer& tableFrom, uint64_t uFrom, uint64_t uCount )
{                                                                                                  assert( (uFrom + uCount) <= tableFrom.get_row_count() ); assert( get_column_count() == tableFrom.get_column_count() );
   if( (get_row_count() + uCount) > m_uReservedRowCount ) row_reserve_add( uCount );
 ...
 * @param tableHarvest table to store harvested data
 */
void table::harvest(const std::vector<uint64_t>& vectorRow, dto::table& tableHarvest) const
{
 ...
 */
void table::harvest(const std::vector< unsigned >& vectorColumn, const std::vector<uint64_t>& vectorRow, dto::table& tableHarvest) const
{
   if( tableHarvest.column_empty() == true )                                   // no columns? the create
   {
