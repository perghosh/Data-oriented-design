...
## v1.1.0
### New Features
   - Override options arguments in history entries with command line arguments. This makes history entries more flexible and reusable for different searches.
   - `clip` clip flag for commands `list` and `find` and this tries to find if path is stored in clipboard and take that file to search in. This speeds up searches within developer tools
   - Similar to a simpler version of jinja templating to be used in history files, entries there can have variables and those can be prompted for when running history entries. This makes it possible to create more dynamic history entries that can be reused for different searches.
   - Combining history with the has_tag method makes it possible to create powerful searches that can be reused often with different tags.
   - Variables in history entries can have description that is shown when prompting for values. This makes is a lot simpler to create advances searches with filters using expressions.
   - A compressed version of key value pair printing using the `find` command. This makes key value pair much more usefull for bookmarking and similar tasks.

## v1.0.9

### Bug Fixes
   - The internal scripting engine had a bug that caused it to not work properly with empty strings. This has been fixed and now the scripting engine works as expected with empty strings.


## v1.0.8
### New Features
- `copy` got an alias `cp` to make it easier to use for linux users.
- `dir` command got `ls` to make it easier to use for linux users.
- `copy` now works with `--pattern`
- `copy` now works with `--rpattern`
- `dir` now works with `--pattern`
- `dir` now works with `--rpattern`
- `preview` added to `copy` to preview what will be copied without actually copying files.
- `detail` options affect more commands now like `dir` and `copy`
- `parents` option added to `dir` for making it possible to click to open for files with same name in different folders.
- `dir` can show file permissions when `detail` level is 3 or higher (full and extended).
- Skanning files now works similar to default `ls` or `dir` commands on linux and windows by showing hidden files and folders by default. Use `--ignore` to ignore files and folders you do not want to see.


## v1.0.7
### New Features
- Much work done to internal script logic to make cleaner a lot more flexible by configure commands with expressions.
- Filter result with WHERE conditions for find, list, dir and count commands. Lots of methods added to create advanced filters.
- Syntax for filtering with where is similar to sql syntax.
- Added support for case-insensitive and whole-word matching in the find command.
- Improved performance of pattern matching by using threads for `find` command.
- Enhanced logging capabilities for better debugging and analysis.
- `copy` command works similar to cp command in linux, but with options to filter files, folders and content.
- `copy` command between two folders that recurses into subfolders generating the same folder structure in target as in source.
- `context` added to find when generating key value output
- `dir` command have gotten more columns
- `help` command improved to show information about how commands and options work together
- Improved logging that can be set using options to cleaner.
- Global option `detail` with four levels on how much detail to show in output

## v1.0.6
### New Features
- Speed improvements for huge count results, it uses a bit more momory though.
- `--add-to-history` options adds command line with name to history file. This allows you to save commands you use often and easily repeat them later.
- `-home` option added to where to open history or config file, home specifies user home directory.
- `--run` in history command tries to find history in current directory first, if not found it looks in user home directory. This makes it easier to use history files for different projects.
- command `copy` added to copy files
- For `copy` the options `--source`, `--target`, `--ignore`, `--filter` and `--newer` was added
- `copy` will generate the same folder structure in target as in source if needed
- `prompt` and `logging-severity` works for saved command where history can execute them
- `alias` added to history items to simplify running them
- `find` is now threaded to improve performance
- `help` formatting improved

bug fixes
- Fixed a bug when help duplicated flags documentation
...
- `kv-where` filter for key value pairs added to the `find` command. This allows you to filter results based on specific key value pairs, making it easier to find relevant information. This is what makes key value pairs really useful.
