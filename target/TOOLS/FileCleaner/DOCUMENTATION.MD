...
Some functions support both namespace and non-namespace calling:
- `length(text)` or `str::length(text)`
- `max(a, b)` or `math::max(a, b)`
...
| `mid` | `mid(text, start, length)` | Extracts substring (1-based start) | `mid("hello", 2, 3)` → `"ell"` |
